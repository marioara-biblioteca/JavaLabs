		...
		while(true) {
			...
			case "add":
				...
				try {
					...
					party.add(heroFactory.createHero(heroType,cmdTokens[2]));
					world.setParty(party);
					System.out.println("Party now:" + world.getParty());
			...
			case "start":
				...
				if (!world.isAdventureStarted()) {
					...
					TreasureDiscoverer treasureDiscoverer=new TreasureDiscoverer();
					WorldPrinter worldPrinter=new WorldPrinter();
					world.addObserver(treasureDiscoverer);
					world.addObserver(worldPrinter);
